...
4. Evitar duplicación en estado.
5. Evitar anidación excesiva de estado.
...
o podrías crear un estado que represente un objeto con todas las variables a actualizar:
...
Si analizamos un poco estos estados nos podemos dar cuenta de que el estado `isSent` (enviado) no podrá ser verdadero a menos que `isSending` (enviando) esté el falso. No se va a dar el caso en que ambos sean verdaderos.
...
Este estado sólo podrá tener uno de estos tres valores: `typing`, `sending` o `send`. De esta forma evitas confundirte entre estados u olvidarte de actualizar alguno de ellos.
...
Con esto te aseguras también que escribas mal algún estado omitiendo alguna letra por ejemplo.
...
Se podría pensar que necesitamos un estado para el nombre comppleto (`fullName`), pero en realidad este estado es redundante, ya que podemos obtener el nombre completo a partir de los otros dos estados (`firstName` y `lastName`).
